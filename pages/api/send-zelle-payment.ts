...
const CHECKBOOK_API_URL = process.env.CHECKBOOK_API_URL || 'https://sandbox.checkbook.io/v3/zelle';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.error('Error sending Zelle payment:', error);
    res.status(500).json({ message: 'Error sending Zelle payment' });
  }
}